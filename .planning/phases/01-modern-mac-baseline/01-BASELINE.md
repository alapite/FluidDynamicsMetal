...
| Drag produces moving blue density | NOT TESTED | No automated pointer interaction or observed pixels in this agent session. |
| Space pauses/resumes | NOT TESTED | Requires observing the running Metal view. |
| S cycles four fields | NOT TESTED | Requires observing the running Metal view. |
| macOS 26 runtime launch/drag | NOT TESTED | Current host runs macOS 27; a macOS 26 host is needed to establish this result. |
...
The original GIF above is the existing visual reference; it is not a screenshot of this build. The app itself contains no screen or audio capture code. The recording permission prompt may have been triggered by the agent's screenshot attempt; **no recording permission is needed** to open the app and test the mouse and keys. Manual drag/Space/S checks are deferred until phase closeout. Record their observed results here when ready; add a screenshot link only if an image already exists without prompting for permissions.
